...
module.exports = (params) => {
  ...
  return publish(params.tablePrefix + "customer_overview", {
...
and ${sql.timestamps.truncate('month', 'created_at')}= ${sql.timestamps.truncate('month', sql.timestamps.currentUTC())}
then amount 
...
then amount 
  ...
  sum(case when ${sql.timestamps.truncate('month', 'created_at')}= ${sql.timestamps.truncate('month', sql.timestamps.currentUTC())}
then amount 
...
then 1 
  ...
  then ${sql.timestamps.truncate('day', 'created_at')}
...
from ${ctx.ref(params.stagingSchema, params.stagingTablePrefix + params.tablePrefix + 'customer')} as customer
